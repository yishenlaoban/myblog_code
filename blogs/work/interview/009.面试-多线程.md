...
使用Unsafe类中的compareAndSwap方法，根据内存地址偏移量获取到内存中此变量的值，与传入的旧期望值比较，如果相等则修改值为新的期望值，不相等则自旋（while）尝试继续修改
...
getAndSet为例子，调用Unsafe类中的 getAndSetInt(AtomicInteger对象，它的value的内存地址偏移量，旧的期望值，要修改的值)
...
/*Unsafe*/
使用do...while尝试修改，如果第一次修改没有成功，则自旋尝试再次修改，直到成功或者发送异常
...
影响：不影响多线程并发的正确性，如果必须要避免ABA,**可以使用xxxStampedReference**（带版本的CAS）或者使用传统的互斥同步（可能比CAS根高效）
...
![image-20220503182256489](https://img.yishenlaoban.top/image_my/2346254-20220503182258685-1557172232.png) 
...
- `AtomicMarkableReference`：原子更新带有标记的引用类型。该类将 boolean 标记与引用关联起来，也可以解决使用 CAS 进行原子更新时**可能**出现的 ABA 问题(降低机率)。
...
```java
class Test {
        private volatile int count = 0;//保证读可见
...
public final void lazySet(int i, int newValue)//最终 将index=i 位置的元素设置为newValue,使用 lazySet 
```
...
:::tip AtomicStampedReference 类使用示例
...
currentValue=666,currentStamp=666,currentStamp=999
...
:::tip AtomicMarkableReference 类使用示例
...
如果需要原子更新某个类里的某个字段时，需要用到**对象的属性**修改类型原子类。
...
:::tip AtomicIntegerFieldUpdater 类使用示例
...
​        如果当前的共享资源是空闲的，则将请求资源的线程设置为有效工作线程，并将资源设置为锁定状态；如果资源被占用，那么需要一套线程阻塞等待和分配资源的机制，这个机制AQS就是使用 队列实现。

:::	

![enter image description here](https://img.yishenlaoban.top/image_my/2346254-20220503203252515-1372432502.png) 

**AQS 使用一个 int 成员变量来表示同步状态，通过FIFO队列来实现线程获取资源的工作，使用CAS来设置获取到资源的线程，和同步状态（保证原子性操作）。**
...
static final class FairSync extends Sync {
    ...
    public final void acquire(int arg) {
        if (!tryAcquire(arg) && //尝试获取资源 false:没获取到
...
> 这里的Semaphore 相当于与一个容器，容器还有空位当前请求资源的线程就拿走一个位子并设置可访问资源，通过`acquire()`方法进行阻塞，当线程执行`release（）`，就信号量加回来（位置空出来）
...
`CountDownLatch`允许 `count`个线程阻塞在`await()`这个地方，直到任务执行完毕

`CountDownLatch`实现共享锁，默认构造 `AQS`的 同步状态state 为 count。 当线程使用`countDown()`时，底层调用 `tryReleaseShared()`方法 CAS 的减少state, 直到 `state`为0。当调用`await()`,线程判断state==0, 如果不为0线程一直阻塞（`await`后面的方法都不会执行），counDownLatch 会CAS自旋的判断 state, state 等于0就会释放等待线程，执行其后面的方法。

:::



:::warning CountDownLatch 的两种典型用法
...
:::warning  CountDownLatch 的不足

CountDownLatch 是一次性的，计数器的值只有在构造器里初始化一次，之后就没有任何的机制再次对其赋值，当CountDownLatch 的值用完了，它不再被使用。
...
:::tip CyclicBarrier 和 CountDownLatch 的区别
...
对于 `CountDownLatch` 来说，重点是“一个线程（多个线程）等待”，而其他的 N 个线程在完成“某件事情”之后，可以终止，也可以等待。而对于 `CyclicBarrier`，重点是多个线程，在任意一个线程没有完成某一阶段的任务，所有的线程都必须等待，等到该完成任务的线程都完成了任务，就在一起执行下一阶段的任务。
